...
outcome variables. In this section, we illustrate the use of our
package for multilevel data by focusing on two types of data
structure: (1) the treatment is assigned at the group level whereas
the mediator and outcome are measured at the individual level, and (2)
both the treatment and mediator are group-level variables while the
outcome is recorded at the individual level. Other combinations of
data levels can also 
be handled via straightforward modifications to the syntax used in
these two types of data structure, and the vignette of the \pkg{mediation} package
contains some examples of the individual-level treatment and mediator with
