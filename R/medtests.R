...
#' 
#' The function takes the original call to \code{mediate} and reruns the
#' algorithm twice with the two sets of \code{covariates} values. It assumes
#' that the objects in the environment in which the original mediate call was
#' made also exist in the current environment under the same variable names,
#' i.e., it evaluates the updated call in the current environment.
#' 
...
#' function returns an object of class "\code{test.modmed.mediate.order}" which
#' is a list composed of "\code{htest.order}" objects.
#' @author Teppei Yamamoto, Massachusetts Institute of Technology,
#' \email{teppei@@mit.edu}.
