...
#' the mediator of interest, exist in the hypothesized causal mechanisms. It
...
#' performs sensitivity analysis with respect to the violation of the
#' homogenous interaction assumption. The function can be used for both the
#' single experiment design and the parallel design.
#' 
#' This function implements the framework proposed by Imai and Yamamoto (2012)
#' for the estimation and sensitivity analysis for multiple causal mechanisms.
#' It estimates the average causal mediation effects (indirect effects) with
#' respect to the mediator of interest ('med.main'), i.e., the portion of the
#' treatment effect on the outcome that is transmitted through that mediator,
#' as well as the average direct effects, i.e., the portion of the treatment
#' effect on the outcome that is not transmitted through the main mediator.
#' Unlike the "standard" causal mediation analysis implemented by
#' \code{\link{mediate}} and \code{\link{medsens}}, this framework allows the
#' existence of post-treatment covariates that confound the relationship
...
#' the treatment and the mediator were randomized), there is no need to specify
#' a particular post-treatment confounder, for any such confounder (observed or
#' unobserved) is allowed to exist by virtue of the design. Similarly, no
#' observed covariates need to be included. The function instead requires an
#' additional variable ('experiment') indicating whether the mediator was
...
#' the assumption that the degree of treatment-mediator interaction is constant
#' across all units. The estimated total treatment effect is also reported.
#' 
...
#' square tilde, the proportion of the total variance explained by such a term.
#' The confidence region is also calculated, using the Imbens and Manski (2004)
#' formula with bootstrap standard errors. Further details are given in the
#' above reference.
#' 
...
#' of 1 and 0 and that both mediators and outcome variable must be numeric. The
#' pre-treatment covariates can be of any type that \code{\link{lm}} can handle
#' as predictors.
#' 
...
#' @param experiment name of the binary indicator whether 'med.main' is
#' randomly manipulated or not. Only used under the parallel design.
...
#' variance explained by heterogeneity in treatment-mediator interactions. Must
...
#' and confidence intervals are evaluated.} \item{R2tilde}{ values of the R
#' square tilde parameter.} \item{R2star}{ values of the R square star
#' parameter.} \item{d1.lb, d0.lb, d.ave.lb}{ lower bounds on the average
#' causal mediation effects under treatment, control, and the simple average of
#' the two, respectively, corresponding to the values of the sensitivity
#' parameters listed above. Note that the first elements of these vectors equal
#' the point estimates under the homogeneous interaction assumption.}
#' \item{d1.ub, d0.ub, d.ave.ub}{ upper bounds on the average causal mediation
#' effects.} \item{d1.ci, d0.ci, d.ave.ci}{ confidence intervals for the
#' average causal mediation effects at different values of the sensitivity
#' parameters.} \item{z1.lb, z0.lb, z.ave.lb}{ lower bounds on the average
#' direct effects under treatment, control, and the simple average of the two,
...
#' estimates under the homogeneous interaction assumption.} \item{z1.ub, z0.ub,
#' z.ave.ub}{ upper bounds on the average direct effects.} \item{z1.ci, z0.ci,
#' z.ave.ci}{ confidence intervals for the average direct effects at different
#' values of the sensitivity parameters.} \item{tau}{ point estimate of the
#' total treatment effect.} \item{tau.ci}{ confidence interval for the total
#' treatment effect.} \item{conf.level}{ confidence level used for the
...
#' 
#' Imai, K. and Yamamoto, T. (2012) Identification and Sensitivity Analysis for
#' Multiple Causal Mechanisms: Revisiting Evidence from Framing Experiments,
...
#' 
#' Function to plot results from \code{multimed}. Most standard plotting
#' options are available.
#' 
#' 'type' and 'tgroup' can contain multiple character strings, in which case
#' multiple plots are produced. For the use of graphical parameters see
...
#' treated ("treated"), control ("control") and the simple average of these two
#' effects ("average").
...
#' the width of confidence bars. For sensitivity plots this is the width of the
#' lines for the bounds.
#' @param pch plotting points used for the "point" plots.
#' @param cex magnification factor for the plotting points in the "point"
#' plots.
