...
#' 
#' This function processes multiple treatment/mediators/outcome variable
#' combinations to produce a collected set of output ready for analysis or
#' graphing. In principle, this is a function designed to facilitate running
#' causal mediation analyses on multiple models that share the same basic
#' specification (i.e. the types of parametric models and the set of
#' pre-treatment covariates) except the treatment, mediator and outcome
...
#' whether different mediators transmit the effect of the treatment variable on
#' a variety of outcome variables. A single set of pre-treatment control
...
#' these will be contained in each data frame, along with the outcome variable.
#' The function will estimate all of the combinations of treatment variables
...
#' \code{\link[=plot.mediate]{plot}} for tabular and graphical summaries of the
#' results. Alternatively, the entire output may be directly passed to
...
#' should be noted that users typically should have pre-treatment covariates to
#' make the sequential ignorability assumption more plausible.
#' 
...
#' outcome variables contained within each data frame. All of the outcomes will
#' be used with each treatment variable and must be in each data frame.
...
#' @return An object of class "mediations" (or "mediations.order" if the
#' outcome model is ordered probit), a list of "mediate" ("mediate.order")
#' objects produced by applications of \code{\link{mediate}} for the specified
...
#' Plotting Indirect, Direct, and Total Effects from Multiple Mediation
#' Analyses
#' 
...
#' figure is plotted.  Default is to ask only if the number of plots on current
#' screen is fewer the number implied by 'which'.
...
#' 
#' The 'summary.mediations' function produces a summary of results from
#' multiple causal analyses conducted via \code{\link{mediations}}.  Output is
#' a series of \code{\link{summary.mediate}} outputs for all the
