...
plot.medsens <- function(x, sens.par = c("rho", "R2"), 
    ...
    ##################################################
        ...
        for(i in 1:length(out)){
            ...
            if(is.null(ylab)){
            	if(wh){
	                title(ylab = bquote(paste(.(o$labs[2]), ": ", bar(delta)(.(o$labs[3])))), 
    	            	  line = 2.5, cex.lab = .9)
            	} else {
	                title(ylab = bquote(paste(.(o$labs[2]), ": ", bar(delta))), 
    	            	  line = 2.5, cex.lab = .9)
            	}
            } else title(ylab = ylab)
		}
