    ...
    sims<-1000 #the sims needs to be fixed
                    ...
                    i <- 1
                            ...
                            for(i in 1:length(rho)){
                                    ...
                                    t <- 0
                                    d0.covartemp <- pnorm((alpha3 + beta3*t + eta3.X + gamma*(alpha2.est + beta2.est + eta2.X))/sqrt(gamma^2*sigma.sq+2*gamma*rho[i]*sigma+1)) -pnorm((alpha3 + beta3*t + eta3.X + gamma*(alpha2.est + eta2.X))/sqrt(gamma^2*sigma.sq+2*gamma*rho[i]*sigma+1))
...
#this ends the sensitivity function
}
