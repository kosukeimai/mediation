...
#' ignorability assumption.  The function takes output from '\code{mediate}'
#' and calculates the true average causal mediation effects and direct effects
#' for different values of the sensitivity parameter representing the degree of
#' the sequential ignorability violation.
#' 
#' This is the workhorse function for sensitivity analyses for average causal
#' mediation effects. The sensitivity analysis can be used to assess the
#' robustness of the findings from \code{mediate} to the violation of
#' sequential ignorability, the crucial identification assumption necessary for
#' the estimates to be valid. The analysis proceeds by quantifying the degree
#' of sequential ignorability violation as the correlation between the error
#' terms of the mediator and outcome models, and then calculating the true
#' values of the average causal mediation effect for given values of this
...
#' direct effect by setting 'effect.type' to "direct" (or "both" if results for
#' the average causal mediation effect are also desired).
#' 
...
#' computational time, as does decreasing 'eps' (for the linear-linear case) or
#' the number of simulations 'sims' (for the binary-linear and linear-binary
#' cases).
#' 
...
#' @param sims the number of Monte Carlo draws for the calculation of
#' confidence intervals. Only used in cases where either the mediator or
...
#' sensitivity parameter rho.} \item{upper.d0, lower.d0, upper.d1,
#' lower.d1}{vectors of upper and lower confidence limits for average causal
#' mediation effect under the control and treatment conditions for each value
#' of rho.} \item{z0, z1}{vectors of point estimates for average direct effect
#' under the control and treatment conditions for each value of sensitivity
#' parameter rho.} \item{upper.z0, lower.z0, upper.z1, lower.z1}{vectors of
#' upper and lower confidence limits for average direct effect under the
#' control and treatment conditions for each value of rho.} \item{tau}{a vector
#' of point estimates for total effect for each value of rho.  Only present
#' when the outcome model is binary.} \item{upper.tau, lower.tau}{vectors of
#' upper and lower confidence limits for total effect.  Only present when the
#' outcome model is binary.} \item{nu}{a vector of point estimates for the
#' proportion mediated for each value of rho.  Only present when the outcome
#' model is binary.} \item{upper.nu, lower.nu}{vectors of upper and lower
#' confidence limits for the proportion mediated.  Only present when the
#' outcome model is binary.} \item{rho}{a numeric vector containing the values
#' of sensitivity parameter rho used.} \item{rho.by}{a numeric value indicating
#' the increment of rho used.} \item{sims}{a numeric value indicating the
#' number of Monte Carlo draws used.} \item{err.cr.d, err.cr.z}{the values of
#' rho with which the average causal mediation and direct effects are zero.
#' Vectors of length two if 'INT' is 'TRUE'; numeric values otherwise.}
...
#' value of rho.} \item{R2star.prod}{a numeric vector containing the values of
#' the products of the two "R square stars", representing the proportions of
#' residual variance in the mediator and outcome explained by the hypothesized
#' unobserved confounder. The values correspond to those of rho. See
#' \code{\link{plot.medsens}} for details.} \item{R2tilde.prod}{a numeric
#' vector containing the values of the products of the two "R square tildes",
#' representing the proportions of total variance in the mediator and outcome
#' explained by the hypothesized unobserved confounder. The values correspond
#' to those of rho. See \code{\link{plot.medsens}} for details.}
...
#' zero, respectively.} \item{R2tilde.d.thresh, R2tilde.z.thresh}{the values of
#' the product of "R square tildes" for which the average causal mediation and
#' direct effects are zero, respectively.} \item{r.square.y, r.square.m}{the
#' usual R square statistics for the outcome and mediator models.} \item{INT}{a
#' logical value indicating whether interaction between the treatment and
#' mediator is allowed in the original mediate object.} \item{conf.level}{the
#' confidence level used.} \item{effect.type}{the 'effect.type' argument used.}
...
#' mediator and binary outcome models).} \item{robustSE}{`TRUE' or `FALSE'.}
...
#' 
#' The sensitivity analysis for causal mediation effects can be conducted in
#' terms of two alternative sensitivity parameters, which both quantify the
#' degree of violation of the sequential ignorability assumption. The "rho"
#' parameter represents the correlation between the two error terms of the
#' (latent) linear models for the mediator and outcome variables. A large value
#' of rho indicates the existence of important common unobserved predictors for
#' both the mediator and outcome and therefore a high degree of sequential
#' ignorability violation, while a value close to zero implies there is no such
#' confounders.
#' 
...
#' The confidence level is determined by the 'conf.level' value of the original
...
#' mediator and outcome. (The name derives from the fact that this direction is
#' mathematically represented by the sign of the product of two regression
#' coefficients.) If "positive" (or a numeric value 1) is given, the confounder
#' is assumed to affect the mediator and outcome in the same direction.  If
#' "negative" (or a numeric value -1), the effect is assumed to be in opposite
#' directions.
#' 
...
#' @param sens.par a character string indicating the sensitivity parameter to
#' be used. Default plots effects as functions of "rho". See Details.
...
#' @param sign.prod a value indicating the direction of hypothesized
#' confounding in the sensitivity analysis. If "positive", the confounder is
#' assumed to affect the mediator and outcome variable in the same direction;
#' if "negative" the effects are assumed to be in opposite directions.  Only
#' relevant if sens.par is set to "R2".
#' @param pr.plot a logical value. If 'TRUE', the "proportions mediated" will
#' be plotted instead of the average causal mediation effects or direct
#' effects.  Currently only available if the object 'medsens' is based on the
#' linear mediator and binary probit outcome models. Default is 'FALSE'.
