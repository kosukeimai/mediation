...
function can now be used for ordered response models fitted via the {\tt polr} or {\tt bayespolr}
...
for a discussion of the implementation.} for some parametric models (e.g., 
...
enabled by specified the variable on which to cluster. The default is to not 
cluster and hence is set to null. 
