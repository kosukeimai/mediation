...
\item Weights can now be used. For example, if researchers have survey weights then these will be extracted and used to estimate the quantities of interest.
...
%\vspace{-.5in}
%\begin{center}
%\includegraphics[scale=.8]{jobs-sens-c.pdf}
%\end{center}
%\vspace{-.5in}
...
%\vspace{-.5in}
%\begin{center}
%\includegraphics[scale=.8]{R2-ContCont-JOBS.pdf}
%\end{center}
%\vspace{-.5in}
%\caption{Sensitivity Analysis with Continuous Outcome and Mediator in terms of proportion of total variance.
%  \label{jobplot2}}
%\end{figure}
...
Researchers may face a situation where they want to conduct multiple causal mediation analyses. For example, the researcher may have run several different experiments that had different treatment conditions but had the same mediator and outcome variables. Or, a researcher may measure several different mediating variables in their experiment and wish to estimate causal mediation effects for each of these cases. In these situations the researcher would need to write separate lines of code for the different models. A more efficient way is too loop over the different combinations of interest. In R this is generally more difficult and so we wrote a separate function, {\tt mediations}, that facilitates this process by making use of the {\tt sprintf} wrapper.

{\tt mediations} can be used to estimate causal mediation analyses over the range of treatment, mediator, and outcome combinations that could be present within a study. For example, if there was two treatment conditions, two measured mediators, and two outcomes, mediations would estimate 8 causal mediation analyses. The results can then be analyzed with the standard {\tt summary} and {\tt plot } functions discussed previously. If there is only one treatment/control condition, the user needs only a single data set that contains all mediators and outcomes of interest. If there are multiple treatment conditions, then the researcher must first create separate data frames, each with all mediator/outcome variables to be considered, and named using a convention where the first part of the data frame name is the name of the treatment variable contained within it.

Use of the mediations function is straightforward. First, the user creates lists of the treatment, mediator, outcome, and covariates names that are to be used. The treatment, mediator, and outcome variable lists are looped over to produce the various combinations of mediator and outcome models. The set of covariates is held constant across the models. The different model combinations are then entered into the mediations function. The user specifies several different options for {\tt mediations}. First, the type of model to be used for the mediator and outcome equations must be specified. Currently, mediations can handle a variety of different model combinations. Either the mediator or outcome equation can be modeled with linear regression, probit, ordered probit, and quantile regression. Mediations also handles the case where tobit is used for the outcome. Next, specifies the standard features that are necessary for the standard mediate function, such as the number of simulations. In addition, if quantile regression or tobit regression are used, the appropriate parameters must also be specified. If the user wishes for estimates to include weights (e.g., survey weights) then the name of the weight variable needs to be passed through the mediations function.

The output of mediations contains a collection of mediate objects. The naming system uses the name of the particular outcome variable, mediator, and treatment, each separated by a ``.''. For example, if the output of mediations is {\tt medations.output} and the outcome, mediator and treatment variable names ``O'', ``M1'' or ``M2'', and ``T'', respectively, then {\tt mediations.output\$O.M2.T} would be the mediate object from using mediator ``M2''.
